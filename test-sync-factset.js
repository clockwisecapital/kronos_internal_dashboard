// Test syncing FactSet data from Google Drive

async function testFactsetSync() {
  console.log('\n' + '='.repeat(70))
  console.log('Testing Google Drive Sync: FactSet')
  console.log('='.repeat(70) + '\n')

  const startTime = Date.now()

  console.log('📋 Sending sync request to /api/google-sync...')
  console.log('   File Type: factset')
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.log('3. Make sure factset_data_v2 table exists in Supabase')
    ...
    console.log('='.repeat(70))
    console.log('1. Check Supabase factset_data_v2 table:')
    console.log('   - Should have ~2,300 records')
    console.log('   - Check a few sample tickers (SPY, QQQ, AAPL, etc.)')
    console.log('\n2. Check the UI at http://localhost:3001/data-sync')
    console.log('   - Sync should appear in history')
    console.log('\n3. Run verify-factset.js to check data integrity\n')
